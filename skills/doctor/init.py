...
5. Generates project-local hook scripts under `<project>/.claude/hooks/`. The plugin no
   longer ships generic typecheck / lint scripts — init detects which typecheck / lint
   tools the project actually uses (parsing package.json / pyproject.toml / Cargo.toml /
   go.mod / deno.json + `shutil.which` on each candidate) and emits minimal, stack-specific
   shell scripts that invoke only those tools. Existing hook files are NEVER overwritten —
   the user may have customised them.
6. Safe-merges `.claude/settings.json`: PostToolUse entries point at the project-local hook
   paths (`.claude/hooks/typecheck.sh`, `.claude/hooks/lint.sh`). Stale entries from any
   prior install (including v0.4 plugin-path entries) are stripped; user-authored keys are
   preserved untouched.
...
# ────────────────────────────────────────────────────────────────────────────
# Project-local hook generation
# ────────────────────────────────────────────────────────────────────────────
#
# Hooks are project-specific scripts emitted into `<project>/.claude/hooks/`. The plugin no
# longer ships generic typecheck.py / lint.sh — instead init detects the stack and writes
# minimal, technology-specific scripts that only invoke the tools the project actually has.
#
# If a hook file already exists at the target path, init leaves it alone — the user owns it
# and may have customised it. settings.json is still wired to point at the path.

HOOK_FILE_HEADER = """\
#!/usr/bin/env bash
# Auto-generated by /sdd:doctor init — __KIND__ hook for __LABEL__.
# Re-run /sdd:doctor init to regenerate, OR edit freely — init will skip a file that exists.
set -euo pipefail

# Read PostToolUse JSON payload from stdin and pull out tool_input.file_path.
...
# Each tool contributes one shell block keyed off `$ext`. The block is emitted only when the
# tool is in the detection result. Blocks are concatenated in detection order.
TYPECHECK_BLOCKS = {
    "tsc": """
case "$ext" in
  ts|tsx)
    command -v npx >/dev/null 2>&1 || exit 0
    if npx --no-install --quiet tsc --version >/dev/null 2>&1; then
...
case "$ext" in
  py)
    ...
    if ! mypy --strict "$file_path" 2>&1; then
...
case "$ext" in
  py)
    ...
    if ! pyright "$file_path" 2>&1; then
      echo "❌ pyright failed for $file_path" >&2
      exit 2
    fi
    ;;
esac
...
case "$ext" in
  ts|tsx|js|jsx|mjs)
    ...
    if ! deno check "$file_path" 2>&1; then
      echo "❌ deno check failed for $file_path" >&2
      exit 2
    fi
    ;;
esac
...
case "$ext" in
  ts|tsx|js|jsx|mjs|cjs)
    ...
    if npx --no-install --quiet @biomejs/biome --version >/dev/null 2>&1; then
      if ! npx --no-install @biomejs/biome check "$file_path" 2>&1; then
...
case "$ext" in
  ts|tsx|js|jsx|mjs|cjs)
    ...
    if npx --no-install --quiet oxlint --version >/dev/null 2>&1; then
      if ! npx --no-install oxlint "$file_path" 2>&1; then
...
case "$ext" in
  go)
    ...
    if ! golangci-lint run --new-from-rev=HEAD~ ./... 2>&1; then
      echo "❌ golangci-lint failed for $file_path" >&2
      exit 2
    fi
    ;;
esac
...
case "$ext" in
  ts|tsx|js|jsx|mjs)
    ...
    if ! deno lint "$file_path" 2>&1; then
      echo "❌ deno lint failed for $file_path" >&2
      exit 2
    fi
    ;;
esac
...
def render_hook_script(kind: str, tool_names: list[str], blocks: dict[str, str]) -> str:
    """Compose a project-local hook script body from per-tool blocks.

    Uses simple `.replace()` rather than `.format()` so the embedded shell `${var}`
    syntax in the template doesn't collide with Python's brace-based formatting.
    """
    label = ", ".join(tool_names) or "no tools"
    header = HOOK_FILE_HEADER.replace("__KIND__", kind).replace("__LABEL__", label)
    parts = [header]
    for name in tool_names:
        snippet = blocks.get(name)
        if snippet:
            parts.append(snippet)
    parts.append(HOOK_FILE_FOOTER)
    return "".join(parts)


def write_project_hook(
    project_root: pathlib.Path,
    relative_path: str,
    content: str,
    *,
    ...
    """Write a hook script under <project>/.claude/hooks/ unless it already exists.

    Returns (status, target_path). status ∈ {"created", "skipped", "would-create"}.
...
def generate_project_hooks(
    project_root: pathlib.Path,
    detected: dict[str, list[str]],
    *,
    dry_run: bool,
) -> list[str]:
    """Emit `<project>/.claude/hooks/typecheck.sh` and `lint.sh` for the detected stack.

    Skips writing each file if it already exists. Returns human-readable status lines.
    """
    lines: list[str] = []

    if detected["typecheck"]:
        content = render_hook_script("typecheck", detected["typecheck"], TYPECHECK_BLOCKS)
        status, target = write_project_hook(
            project_root, ".claude/hooks/typecheck.sh", content, dry_run=dry_run
        )
        suffix = ", ".join(detected["typecheck"])
        lines.append(f"  hooks/typecheck.sh   — {status} [{suffix}]: {target}")

    if detected["lint"]:
        content = render_hook_script("lint", detected["lint"], LINT_BLOCKS)
        status, target = write_project_hook(
            project_root, ".claude/hooks/lint.sh", content, dry_run=dry_run
        )
        suffix = ", ".join(detected["lint"])
        lines.append(f"  hooks/lint.sh        — {status} [{suffix}]: {target}")
...
# Substring markers that identify an SDD-managed PostToolUse hook entry. Re-init strips any
# entry whose command contains one of these markers and re-adds fresh ones built from current
# detection. Older v0.4 installs wrote `<plugin>/hooks/typecheck.py` / `lint.sh` paths; the
# new layout writes `.claude/hooks/typecheck.sh` / `lint.sh`. Both are matched so a re-init
# cleans up legacy entries on upgrade.
SDD_HOOK_MARKERS = (
    ".claude/hooks/typecheck.sh",
    ".claude/hooks/lint.sh",
    "hooks/typecheck.py",  # legacy v0.4 plugin-path entries
    "hooks/lint.sh",       # legacy v0.4 plugin-path entries
)
...
def build_hook_entries(
    ...
    detected — in that case caller should not add anything to settings.json. The entries
    invoke project-local hook scripts under `.claude/hooks/` (paths are relative to the
    project root; Claude Code runs hooks with the project root as cwd).
    """
    detected = detect_hookable_tools(project_root, stack)
    inner: list[dict] = []
    if detected["typecheck"]:
        inner.append({"type": "command", "command": "bash .claude/hooks/typecheck.sh"})
    if detected["lint"]:
        inner.append({"type": "command", "command": "bash .claude/hooks/lint.sh"})
    if not inner:
        return [], detected
...
def setup_settings_json(
    ...
    *,
    ...
    new_entries, detected = build_hook_entries(stack, project_root)
...
def main() -> int:
    ...
    # 4. Project-local hook scripts — emit minimal stack-specific .claude/hooks/*.sh files
    #    (skipped if a hook already exists; the user owns it).
    stack = detect_stack(project_root)
    detected_tools = detect_hookable_tools(project_root, stack)
    for line in generate_project_hooks(project_root, detected_tools, dry_run=args.dry_run):
        print(line)

    # 5. .claude/settings.json — wire PostToolUse to the project-local hook paths, safe-merge.
    for line in setup_settings_json(project_root, stack, dry_run=args.dry_run):
        print(line)
