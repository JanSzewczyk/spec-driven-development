...
1. Make changes inside the plugin (`commands/`, `agents/`, `skills/`, `skills/doctor/templates/`, or the hook-snippet tables in `skills/doctor/init.py`).
2. Run the smoke test against the plugin from an empty target project:
   ...
   Expected for an empty project: checks 1, 2, 5 fail (per-project files absent); check 6 warns (no settings.json yet); checks 3, 4 pass if the plugin path resolves; 7, 8, 10 warn or pass depending on environment; 9 depends on your plugin marketplace.
...
| Support for a new typecheck/lint tool | Add a `TYPECHECK_BLOCKS` / `LINT_BLOCKS` entry in `skills/doctor/init.py` (shell snippet keyed by tool name), then teach `detect_hookable_tools()` to recognise the tool's config marker AND verify it on `$PATH`. `/sdd:doctor init` will compose it into the project-local hook script automatically. |
